...
#import "VoiceCommonHelper.h"



@implementation VoiceCommonHelper
...
+ (void)removeRecordedFileWithOnlyName:(NSString*)fileName block:(DidDeleteAudioFileBlock)block {
    ...
    [VoiceCommonHelper removeRecordedFile:fileName type:@"amr"];
    
    NSString *originWavFile = [fileName stringByReplacingOccurrencesOfString:@"wavToAmr" withString:@""];
    [VoiceCommonHelper removeRecordedFile:originWavFile type:@"wav"];
    
    NSString *convertedWavFile = [originWavFile stringByAppendingString:@"amrToWav"];
    [VoiceCommonHelper removeRecordedFile:convertedWavFile type:@"amr"];
...
+ (void)removeAudioFile:(NSString*)exception block:(DidDeleteAudioFileBlock)block {
    NSString *path = [VoiceCommonHelper getCacheDirectory];
...
{
    NSString* fileDirectory = [[[VoiceCommonHelper getCacheDirectory] stringByAppendingPathComponent:_fileName] stringByAppendingPathExtension:_type];
...
{
    NSString* fileDirectory = [[VoiceCommonHelper getCacheDirectory]stringByAppendingPathComponent:_fileName];
    return fileDirectory;
}
...
+ (void)removeRecordedFile:(NSString*)fileName type:(NSString*)type {
    ...
    NSString *path = [VoiceCommonHelper getPathByFileName:fileName ofType:type];
