...
+ (CGFloat)calculateCellHeightWithMessage:(id <XHMessageModel>)message
    ...
    CGFloat userInfoNeedHeight = kXHAvatarPaddingY + kAvatarImageSize + (message.shouldShowUserName ? kXHUserNameLabelHeight : 0) + kXHAvatarPaddingY + timestampHeight;
    ...
    if (self) {
        ...
        switch (message.bubbleMessageType) {
            ...
            case BubbleMessageTypeSending:
                avatarButtonFrame = CGRectMake(CGRectGetWidth(self.bounds) - kAvatarImageSize - kXHAvatarPaddingX,
                                               kXHAvatarPaddingY + (self.displayTimestamp ? (kXHTimeStampLabelHeight + kXHLabelPadding * 2) : 0),
                                               kAvatarImageSize,
                                               kAvatarImageSize);
                break;
...
- (void)layoutSubviews {
    ...
    avatarButtonFrame.origin.x = ([self bubbleMessageType] == BubbleMessageTypeReceiving) ? kXHAvatarPaddingX : ((CGRectGetWidth(self.bounds) - kXHAvatarPaddingX - kAvatarImageSize));
    ...
    if ([self bubbleMessageType] == BubbleMessageTypeReceiving) {
        bubbleX = kAvatarImageSize + kXHAvatarPaddingX * 2;
    } else {
        offsetX = kAvatarImageSize + kXHAvatarPaddingX * 2;
    }
